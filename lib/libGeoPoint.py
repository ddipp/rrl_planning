...
class GeoPoint():
    ...
    def nextpoint(self, azimuth: float, distance: float):
        ...
        drs = m.sin(dr)
        drc = m.cos(dr)
        startLatCos = m.cos(self.rlatitude)
        startLatSin = m.sin(self.rlatitude)

        endLatRads = m.asin((startLatSin * drc) + (startLatCos * drs * m.cos(m.radians(azimuth))))

        endLonRads = self.rlongitude + m.atan2(m.sin(m.radians(azimuth)) * drs * startLatCos,
                                               drc - startLatSin * m.sin(endLatRads))

        return GeoPoint(latitude=m.degrees(endLatRads), longitude=m.degrees(endLonRads))
